...
public class TurtlePond implements KeyEventDispatcher {
	...
	void setup() {
		...
		JOptionPane.showMessageDialog(null, "Move the turtle with the arrow keys to find"
				+ " the hidden cookie.You'll get warmer the closer you get.Hurry or "
				+ "she'll die of hunger!");
...
if(Math.abs(tortoiseLocationX-cookieX) <101 && Math.abs(tortoiseLocationY-cookieY) < 101){
	setBackgroundColor(Color.yellow);
}
		// 8. If the Tortoise is within 50 pixels of the food, set the background color to orange
if(Math.abs(tortoiseLocationX-cookieX) <51 && Math.abs(tortoiseLocationY-cookieY) < 51){
	setBackgroundColor(Color.orange);
}
		// 9. If the Tortoise is within 20 pixels of the food, set the background color to red
if(Math.abs(tortoiseLocationX-cookieX) <21 && Math.abs(tortoiseLocationY-cookieY) < 21){
	setBackgroundColor(Color.red);
}
		// 10. If the Tortoise is within 5 pixels of the cookie, make a pop-up to tell them they found it
if(Math.abs(tortoiseLocationX-cookieX) <6 && Math.abs(tortoiseLocationY-cookieY) < 6){
